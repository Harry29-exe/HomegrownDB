...
func NewTuple(values [][]byte, pattern TuplePattern, tx tx.Tx) Tuple {
	...
	for i, value := range values {
		if value == nil {
			tuple.SetIsNull(reldef.Order(i))
			continue
		}
...
func (t Tuple) IsNull(id reldef.Order) bool {
	var byteNumber = id / 8
...
func (t Tuple) SetIsNull(id reldef.Order) {
	byteNumber := id / 8
...
func (t Tuple) SetIsNotNull(id reldef.Order) {
	byteNumber := id / 8
	bytePos := toNullBitmap + byteNumber
...
func (t Tuple) ColValue(id reldef.Order) []byte {
	if t.IsNull(id) {
		return nil
	}
...
func (t tupleDebugger) stringifyColumnValues(tuple Tuple, arr *strutils.StrArray) {
	...
	for i := 0; i < len(pattern.Columns); i++ {
		...
		if tuple.IsNull(reldef.Order(i)) {
