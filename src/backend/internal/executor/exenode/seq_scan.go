...
func (s *SeqScan) Next() dbbs2.QRow {
	tag := dbbs2.PageTag{PageId: s.page, Relation: s.tableDef.TableId()}
...
func (s *SeqScan) All() []dbbs2.QRow {
	tuplesPerPageEstimate := uint32(dbbs2.PageSize) / (uint32(s.tableDef.ColumnCount()) * 5)
