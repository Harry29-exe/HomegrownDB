...
func (t *basicTokenizer) tokenizeNumber() (tk token.Token, err error) {
	for _, char := range t.futureToken {
		if char == '.' {
			tk, err = token.NewFloatToken(string(t.futureToken))
			return
		}
	}
	tk, err = token.NewIntegerToken(string(t.futureToken))
	return
}
