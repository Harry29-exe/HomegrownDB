...
func newTestTokenSource(query string) *testTokenSource {
	return &testTokenSource{
		TokenCache:  make([]token2.Token, 0, 10),
		CurrentLen:  0,
		Pointer:     0,
...
func (t *testTokenSource) Get(index uint) token2.Token {
	return t.TokenCache[index]
}

func (t *testTokenSource) GetPtrRelative(index int) token2.Token {
	return t.TokenCache[int(t.Pointer)+index]
}

func (t *testTokenSource) Next() token2.Token {
	t.Pointer++
...
func (t *testTokenSource) Prev() token2.Token {
	if t.Pointer < 0 {
		return token2.NilToken()
	}
...
func (t *testTokenSource) Current() token2.Token {
...
func (t *testTokenSource) History() []token2.Token {
