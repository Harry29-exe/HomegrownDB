...
func (s _select) Parse(src tokenizer.TokenSource, v tkValidator) (pnode.SelectStmt, error) {
	src.Checkpoint()
...
func (s stdSelect) parseFullSelect(src tokenizer.TokenSource, v tkValidator) (pnode.SelectStmt, error) {
	src.Checkpoint()
	// Select
	selectNode := pnode.NewSelectStmt()
	...
	selectNode.From = []pnode.Node{rangeVar}
